            ...
            switch ($Operation) {
				"Resize" {
					...
					if ($cbkeepaspect.IsChecked)
					{
						try
						{
							& "$MagickExecutablePath" mogrify -quiet -resize $global:Width'x'$global:Height "$ImageFilePath"
						}
						catch
						{
							continue
				...
				"ResizeCrop" {
					...
					$newaspect = "$global:width`:$global:height"
					$Gravity = $ExtraParameters[2]
					try
					{
						& "$MagickExecutablePath" mogrify -gravity $Gravity -background none -extent $newaspect "$ImageFilePath"
					}
					catch
					{
						continue
...
function ToolResize
{
    ...
    $global:Width = [int]$ExtraParameters[0]
    $global:Height = [int]$ExtraParameters[1]
